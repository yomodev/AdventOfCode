...
namespace AdventOfCode2020CS
{
    public class Day11
    {
        ...
        private static char AlgorithmPart2(int x, int y, int step, string[] s)
        {
            var c = s[y][x];
            var p = new string(' ', 8).ToArray();
            p[0] = Find(x, y, step, s, -1, -1);
            p[1] = Find(x, y, step, s, 0, -1);
            p[2] = Find(x, y, step, s, 1, -1);
            p[3] = Find(x, y, step, s, -1, 0);
            p[4] = Find(x, y, step, s, 1, 0);
            p[5] = Find(x, y, step, s, -1, 1);
            p[6] = Find(x, y, step, s, 0, 1);
            p[7] = Find(x, y, step, s, 1, 1);
        ...
        private static char Find(int x, int y, int step, string[] s, int dx, int dy)
        {
            var w = s[0].Length - 1;
            var h = s.Length - 1;
